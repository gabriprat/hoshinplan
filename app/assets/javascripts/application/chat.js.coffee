$(document).ready ->
  ...
  if form.length < 1
    return;
  channel = '/comment' + form.attr('action').replace('/hoshin_comments','')
  ...
  client.subscribe channel, (payload)->
    ...
    if document.getElementById('comment-' + payload.typed_id) == null
      body = '<li class="item" data-rapid-context="' + payload.typed_id + '"><div id="comment-' + payload.typed_id + '"><a class="user-link" href="' + payload.creator_link + '" rel="nofollow"><span class="view user-name ">' + payload.creator_name + '</span></a> ' + payload.created_at_ago + '<div class="view hoshin-comment-body ">' + payload.body.replace(/</g,"&gt;") + '</div></div></li>'
      $('#comments-part ul').prepend(body);
    button.val(defText)
