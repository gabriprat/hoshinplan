...
(function ($) {
    ...
    var methods = {
        ...
        handleFile: function (file) {
            ...
            // if the file is not of the apropriate type, continue
            if (!file.type.match(options.filetype)) {
                return;
            }
            ...
            var roptions = that.hjq('buildRequest', {
                type: 'post',
                attrs: {ajax: true, complete: methods.complete, error: error}
            });
            roptions.data[options.inputName] = file;
        ...
        setSelect: function () {
            try {
                if (!window.jcrop_api && retried <= options.retries) {
                    retried++;
                    setTimeout(methods.setSelect, 50);
                }
