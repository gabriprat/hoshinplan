class SamlProvider < AuthProvider
  ...
  attr_accessible :issuer, :sso_url, :cert, :fingerprint, :id_format, :metadata_xml, :callback_url
  attr_accessor :metadata_xml, type: :string

  after_save :reload_routes

  def reload_routes
    SamlDynamicRouter.reload
  end
  ...
  def options
    {
        :name => 'saml_' + self.email_domain,
        :assertion_consumer_service_url => callback_url,
        :issuer => self.issuer,
        :idp_sso_target_url => self.sso_url,
        :idp_sso_target_url_runtime_params => {:original_request_param => :mapped_idp_param},
        :idp_cert => self.cert.to_s,
        :idp_cert_fingerprint => self.fingerprint,
        :idp_cert_fingerprint_validator => lambda {|fingerprint| fingerprint},
        :name_identifier_format => self.id_format
    }
  end

  def login_url
    (Rails.configuration.ssl_disable ? 'http' : 'https') + '://' + Rails.application.routes.default_url_options[:host] + "/auth/saml_" + self.email_domain if self.email_domain
  end

  def callback_url
    login_url + "/callback" if self.email_domain
  end

  def metadata_url
    login_url + "/metadata" if self.email_domain
  end
end
