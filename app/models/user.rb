class User < ApplicationRecord
  ...
  before_save do |user|
    if user.invitation_code.present?
      ...
      if ic
        ...
        user.trial_ends_at = user.created_at + trial_days.days if trial_days && !user.trial_ends_at_changed?
  ...
  def add_default_companies_by_domain
  ...
  lifecycle do
    ...
    create :from_omniauth, :params => [:name, :email_address], :become => :active do
      num_companies = add_default_companies_by_domain
      UserCompanyMailer.welcome(self).deliver_later unless num_companies > 0
    end
