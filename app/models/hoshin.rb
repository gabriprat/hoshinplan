class Hoshin < ActiveRecord::Base
  ...
  def completed_tasks
    tasks.completed.where("deadline > current_date - interval '110 day'").order(:lane_pos)
  end
  def discarded_tasks
    tasks.discarded.where("deadline > current_date - interval '110 day'").order(:lane_pos)
  end
  def outdated_tasks
    tasks.overdue
  end
